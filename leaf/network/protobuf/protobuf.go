...
import (
	...
	"github.com/zhanglifan/leaf_server/src/proto/Base"
)
...
func NewProcessor() *Processor {
	...
	p.msgNameMap = make(map[string]*MsgInfo)
	return p
}
...
// It's dangerous to call the method on routing or marshaling (unmarshaling)
func (p *Processor) Register(msg proto.Message, msgName string) uint16 {
...
// goroutine safe
func (p *Processor) UnmarshalBase(data []byte) (interface{}, error) {
	/*
		message BaseMsg {
	...
	*/
	...
	err := proto.UnmarshalMerge(data[:], &baseMsg)
	if err != nil {
		return nil, err
	}
	msgName := baseMsg.GetMsgName()
	msgData := baseMsg.GetMsgBody()
	...
	if i.msgRawHandler != nil {
		...
		return msg, proto.UnmarshalMerge(msgData[:], msg.(proto.Message))
	}
}
